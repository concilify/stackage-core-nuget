...
namespace Stackage.Core.Tests.DefaultMiddleware.ForwardedHeaders.BehindProxyTrue
{
   public class with_headers : middleware_scenario
   {
      ...
      {
         _scheme.ShouldBe("forwarded-proto");
      }

      [Test]
      public void should_make_host_available()
      {
         _host.ToString().ShouldBe("forwarded-host");
      }

      [Test]
      public void should_make_remote_ip_address_available()
      {
         _remoteIpAddress.ToString().ShouldBe("1.2.3.5");
      }
   }
}
